...
function format ( d,data ) {
	...
	$(document).ready(function() {
		...
		$("#ledgerReportTable tr td:first-child").html('<img src="http://i.imgur.com/SD7Dz.png">');
		// Array to track the ids of the details displayed rows
		var detailRows = [];

		$('#ledgerReportTable tbody').on('click', 'tr td:first-child', function() {
			...
			if (row.child.isShown()) {
				...
				for(i=0;i<row.data().ledgerReportDetailTOList.length;i++){
					row.child(format(row.data(),row.data().ledgerReportDetailTOList[i])).show();
				}
				  console.log("LIST LENGTH : "+row.data().ledgerReportDetailTOList.length+"\n");
				  console.log("0th ELEMENT : "+row.data().ledgerReportDetailTOList[0].date)+"\n";
